...
| 方式 | 誰が何をするか | Bot がやること |
|------|----------------|----------------|
| **請求**（既定・推奨） | Bot が請求リンクを発行 → 購入者が支払う | 取引履歴を照合するだけ。**受け取り操作をしない** |
| **送金リンク**（従来） | 購入者が送金リンクを作る | `acceptP2PSendMoneyLink` で受け取る |

請求方式は Bot 側が「受け取る」APIを叩かないため、そのAPIが制限された
アカウントでも成立する可能性がある。購入画面は請求方式で案内するが、
送金リンクを送られた場合も自動的に従来方式で処理する。

### 取引番号はそのまま信用しない（重要）

購入者が送る取引番号は「**どの取引を見るか**」の指定にしか使わない。
商品を渡すかどうかは Bot が `getPaymentHistory` で取得した履歴だけで判断する:

1. 自分の履歴に存在するか（無ければ `TRANSACTION_NOT_FOUND`）
2. 入金方向か（`P2P_CODE_RECEPTION` 等。違えば `TRANSACTION_NOT_INCOMING`）
3. 完了しているか（`TRANSACTION_NOT_COMPLETED`）
4. 金額が注文と**完全一致**か（`AMOUNT_MISMATCH`）
5. 他の注文で使用済みでないか（`LINK_ALREADY_USED`／DBのUNIQUE制約で担保）

不明な `orderType` は**出金扱い**にする（証明できない入金で商品を渡さないため）。
...
- 商品を押すと**詳細画面**（説明・数量別の価格表・在庫）が開く
- **数量ボタン**（1個 / 2個 / 3個 …、価格表に登録した個数ぶん並ぶ）または
  「🔢 数量を入力」で任意個数を選ぶ → **一度に複数個購入できる**
- 価格は**個数ごとに合計金額**で決まる（1個500円・3個1300円のように、
  単価×個数では作れない金額も設定できる）。価格表にない個数は
  直近下の設定から日割り計算される
- 管理者が「まとめ買いボタン」をオフにした商品は、ボタンが「1個」と
  「🔢 数量を入力」だけになる（数量を入力すれば価格表はそのまま適用される）
- 数量を選ぶと Bot が**その金額ぴったりの請求リンク**を発行する
- 購入者はそのリンクから支払い、PayPayアプリに表示される**取引番号**を送信
- Bot が自分の取引履歴（直近10件）を照合し、一致すれば商品を自動配布
- 購入者が代わりに **PayPay 送金リンク**を送ってきた場合は、従来どおり
  Bot が受け取って配布する（どちらを送っても通る）

### 管理者（`ADMIN_TELEGRAM_ID` と一致する場合のみ）

**ボタン操作パネル（推奨・コマンド入力不要）**
`/admin` を送るとインラインボタンのパネルが開きます。以降はタップだけで操作できます:
- 🛍 商品管理 … ➕商品追加 / 📦在庫追加 / 📝注意事項設定 / 💹価格設定(個数ごとの合計金額) /
  🔢まとめ買いボタン(表示・非表示) / 🗑商品削除
...
/product_tiers 1 1:500,2:900,3:1300,10:4000       # 個数ごとの合計金額
/product_bulk 1 off                              # まとめ買いボタンを非表示(on で表示)
...
8. **購入者の自己申告で入金を認定しない**。取引番号は参照キーにすぎず、
   成立判断は必ず `getPaymentHistory` の結果から行う。
9. デバイスUUIDはアカウントごとに固定（毎回新規だと2FA必須＋不正検知に触れる）。
   保存は暗号化し、キーは電話番号の鍵付きダイジェスト（電話番号自体は保存しない）。
...
- 取引番号の照合 → `payment_service.confirm_by_transaction`
- 取引種別（入金/出金）の表 → `src/paypay/constants.py`
- 請求リンク・取引履歴のAPI → `client.create_request_link`, `client.payment_history`
